...
class TestInvoker
  ...
  def setup_and_invoke(tests, context=TEST_SYM, options={:force_run => true})
    ...
    flows = @tests.map do |test|
      setup_f = Concurrent::dataflow_with(Executors.compile) do
        setup_test(test, context, options) 
      end
      
      invoke_f = Concurrent::dataflow_with(Executors.execute, setup_f) do |results|
         invoke_test(results, test, context)
      end

      Concurrent::dataflow_with(Executors.compile, setup_f, invoke_f) do |results, _| 
